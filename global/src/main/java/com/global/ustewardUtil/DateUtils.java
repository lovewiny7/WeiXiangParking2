...
public class DateUtils {
    ...
    public final static String timeFormatWithYMD = "yyyy年MM月dd日";// 时间格式
    public final static String timeFormatWithYMD1 = "yyyy-MM-dd";// 时间格式
    public final static String timeFormatWithMD = "M-d";// 时间格式
    public final static String timeFormatWithMD1 = "M月d日";// 时间格式
    public final static String timeFormatWithYM = "yyyy-MM";// 时间格式
     ...
     * @return String
     */
    public static String getStateTimeNoHours() {
        SimpleDateFormat sdf = new SimpleDateFormat(timeFormatWithYMD);
        Calendar c = Calendar.getInstance();
        Date monday = c.getTime();
        String preMonday = sdf.format(monday);
        return preMonday;
     ...
     * @return 结束时间大于开始时间返回true，否则反之֮
     */
    public static long subTwoTime(String starTime, String endString) {
        SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm");
        try {
            Date parse = dateFormat.parse(starTime);
            Date parse1 = dateFormat.parse(endString);
            
            return parse1.getTime() - parse.getTime();
        } catch (ParseException e) {
            e.printStackTrace();
        }
     ...
     * @return 结束时间大于开始时间返回true，否则反之֮
     */
    public static long subTwoTime(Date starTime, Date endTime) {
        return starTime.getTime() - endTime.getTime();
     ...
     * @throws ParseException
     */
    public static String longToStringWithYMD(long currentTime) {
        String strTime = "";
        if (currentTime != 0) {
            try {
                Date date = longToDate(currentTime, timeFormatWithYMD);// long类型转成Date类型
                strTime = dateToString(date, timeFormatWithYMD); // date类型转成String
            } catch (ParseException e) {
                e.printStackTrace();
            }
        }
